...
summary(fixed)
```
From the above results, we can conclude that variable inflation is insignificant, whereas variable Unemployment, `GDP per capita growth`, `GDP growth` , Population. Moreover we can see that p-value: 2.6547e-15 is less than significance level of 0.05 thus rejects the null hypothesis. 
...
# Perform Chow test
As we can see that the p-value is 0.4443 which means we accept the null hypothesis, which is quite different from the actual results in the research paper. As there siginificance value is less than 0.05 thus accepts the null hypothesis.
